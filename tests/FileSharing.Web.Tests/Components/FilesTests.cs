...
public class FilesTests
{
    ...
    public void RendersEachFileWithItsOwnStatusLabel()
    {
        ...
        var cut = ctx.RenderComponent<Files>();
    ...
    // Toast-on-download moved to Shared/NotificationInboxBridge (mounted once in MainLayout,
    // app-wide) — see NotificationInboxBridgeTests.cs.
    ...
    [Fact]
    public void FilterActive_ShowsOnlyActiveFiles()
    {
        using var ctx = new WebComponentTestContext(DefaultResponder);
        var cut = ctx.RenderComponent<Files>();

        cut.Find("#filter-active").Change(true);

        Assert.Contains("active.pdf", cut.Markup);
        Assert.DoesNotContain("expired.pdf", cut.Markup);
    }

    [Fact]
    public void FilterExpired_ShowsOnlyExpiredFiles()
    {
        using var ctx = new WebComponentTestContext(DefaultResponder);
        var cut = ctx.RenderComponent<Files>();

        cut.Find("#filter-expired").Change(true);

        Assert.Contains("expired.pdf", cut.Markup);
        Assert.DoesNotContain("active.pdf", cut.Markup);
    }

    [Fact]
    public void FilterAll_ShowsEveryFile()
    {
        using var ctx = new WebComponentTestContext(DefaultResponder);
        var cut = ctx.RenderComponent<Files>();

        cut.Find("#filter-expired").Change(true);
        cut.Find("#filter-all").Change(true);
