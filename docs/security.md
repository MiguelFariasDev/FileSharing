...
Documentação da segurança implementada até a Etapa 5 (Autenticação/JWT + Upload de arquivos + Link público de acesso + Download + histórico de downloads). Notificações (SignalR), dashboard e expiração automática via job em background (Hangfire) serão documentados nas etapas correspondentes.
